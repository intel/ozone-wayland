...
WaylandDispatcher::WaylandDispatcher(WaylandDisplay* display)
...
void WaylandDispatcher::PostTask(Task type)
{
    ...
    switch(type) {
      case (Flush):
        message_loop_proxy()->PostTask(FROM_HERE, base::Bind(&WaylandDispatcher::HandleFlush));
        break;
      default:
        break;
    }
...
void WaylandDispatcher::StopPolling()
{
    if (!IsRunning())
    return;
...
void WaylandDispatcher::PostTaskOnMainLoop(const tracked_objects::Location& from_here, const base::Closure& task)
{
...
void WaylandDispatcher::DispatchEvent(scoped_ptr<ui::Event> event) {
    PostTaskOnMainLoop(FROM_HERE, base::Bind(&DispatchEventHelper, base::Passed(&event)));
}
